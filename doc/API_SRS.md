...
**Purpose:** Bulk-upload monthly department reports for a store, month, and year. Existing reports for the same store/month/year are deleted and replaced after the uploaded file is successfully validated.
...
| `reportMonth` | `integer` | ✅       | Target month, from `1` to `12`  |
| `reportYear`  | `integer` | ✅       | Target year, for example `2026` |
| `file`        | `file`    | ✅       | Monthly report Excel file       |
...
The uploaded filename must match one of these formats:

````text
monthly_<reportMonth>_<reportYear>.xlsx
monthly_<reportMonth>_<reportYear>.xls
